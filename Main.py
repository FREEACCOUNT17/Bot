...
# Captcha we Duýduryşlar üçin wagtlaýyn maglumatlar
user_captchas = {}
user_warns = {}
...
# 3. ISHLETMEDIKI AKYM (Threading & Webhook Cleanup)
if __name__ == '__main__':
    # Flask-y aýratyn akymda işletmek
    flask_thread = threading.Thread(target=run_flask)
    flask_thread.daemon = True
    flask_thread.start()

    # Telegram API-däki köne webhook we garashyp duran habarlary doly arassalamak
    try:
        bot.delete_webhook(drop_pending_updates=True)
        time.sleep(2)
    except Exception as e:
        print(f"Webhook arassalanmady: {e}")
