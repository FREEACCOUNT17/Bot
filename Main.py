...
def select_language(message):
    ...
    bot.reply_to(message, get_txt(message.chat.id, 'select_lang'), reply_markup=markup)
...
# P2P EL BAHALARYNY ALJY (Bybit / KuCoin P2P API Adyndan)
...
def get_crypto_prices(message):
    try:
        # Biržalaryň P2P bazaryndan real satylýan ortaça bahalary çekýäris
...
def callback_unmute(call):
    if not is_admin(call.message.chat.id, call.from_user.id):
        bot.answer_callback_query(call.id, "Diňe adminler üçin! / Faqat adminlar uchun!", show_alert=True)
        return
